...
<title>Update Repairs Here</title>
<style>
table, th, td {
...
<body>
	<section class="repair">
		<c:url var="formAction" value="/repair" />
		<form method="POST" action="${formAction}">
			<input type="hidden" name="CSRF_TOKEN" value="${CSRF_TOKEN}" />
			<h1>Update Repair</h1>
			<div class="repair">
				<div class="listOfRepairs">
					<c:choose>
						<c:when test="${not empty repairOrders}">
								<table width=100%>
									<tr>
							...
							<c:forEach var="repairOrders" items="${repairOrders}">
									<tr>
										<td><input type="checkbox" name="selected"
											value="${repairOrders.potholeId}" /></td>
										<td><c:out value="${repairOrders.potholeId}" /></td>
										<td><c:out value="${repairOrders.dateInspected}" /></td>
										<td><c:out value="${repairOrders.rank}" /></td>
										<td><c:out value="${repairOrders.description}" /></td>
										<td><select name="dateRepaired">
				...
				<div class="repair-button">
					...
					<input type="submit" name="btn" value="Repair"
