...
public class CampgroundCLI {
	...
	private static final String[] PARK_MENU_OPTIONS = { PARK_MENU_OPTION_LIST_CAMPGROUNDS,
			PARK_MENU_OPTION_RESERVATIONS, PARK_MENU_OPTION_SEARCH_CAMPSITES,
			PARK_MENU_OPTION_ADVANCED_SEARCH_CAMPSITES, SUB_MENU_OPTION_EXIT };
			...
			if (desiredCampgroundId == 0) {
				...
				if (isCampgroundOpen(campgrounds.get(desiredCampgroundId), dateRange)) {
					...
					if (isAdvanced) {
						...
						campsites = jdbcCampsiteDAO.getCampsiteByCampgroundAdvanced(
								campgrounds.get(desiredCampgroundId), dateRange, campsite);
						// Returns a List of the campsites which fit those requirements and are
						// available in that range of dates
					} else {
						campsites = jdbcCampsiteDAO.getCampsiteByCampground(campgrounds.get(desiredCampgroundId),
								dateRange);
					...
					if (campsites.size() == 0) {
						System.out.print(
								"No campsites available within requested range of dates, would you like to enter a "
										+ "different one (y/n)? ");
						...
						if (campsiteId != 0) { // If campsiteId is 0, then user is skipping reserving a spot
							...
							jdbcReservationDAO.makeReservation(reservationName, campsiteId, dateRange); // Saves a
																										// reservation
																										// to the
																										// database
						}
	...
	private DateRange getRangeOfDatesFromUser() {
		...
		while (true) {
			...
			if (dateRange.getStartDate().isAfter(LocalDate.now())) { // The user is not allowed to reserve spots from
																		// before today
				break;
			} else {
		...
		while (true) {
			...
			if (dateRange.getEndDate().isAfter(dateRange.getStartDate())) { // If the user decides to leave before they
																			// show up, just don't let them do that
				break;
			} else {
	...
	private int getValidCampsiteChoice(List<Campsite> campsites) {
		...
		while (!isAValidChoice) {
			System.out.print("Please choose a campsite (0 to cancel)?");
	...
	private Campsite getDesiredOptionsForCampsite() {
